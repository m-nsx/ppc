...
def priority_traffic_gen(stop):
    ...
    try:
        while not stop.is_set():
            ...
            # Envoi du signal SIGUSER1 pour indiquer la présence d'un véhicule prioritaire au processus lights
            try:
                os.kill(common.LIGHTS_PID, sg.SIGUSER1)
            except ProcessLookupError:
                print(f"\033[91m[ERREUR][ptg] Processus de PID {common.LIGHTS_PID} non trouvé\033[0m")
            except:
                print("\033[91m[ERREUR][ptg] Erreur lors de l'envoi du signal SIGUSER1\033[0m")
